...
export default function GenerateRefCode() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error("Failed to generate referral link at this time");
      }
          ...
          <div>
            ...
            <p className="text-xs text-white/70">
              ...
              <span className="text-amber-300">
                <span className="font-bold">Note:</span> Referral link can not
                be retrieved after generation so store it in a secure location.
              </span>
            </p>
          </div>
          {!generatedUuid && (
            <div>
              <form
                className="flex border justify-between border-accent rounded-sm"
                onSubmit={handleSubmit(onSubmit)}
              >
                <input
                  type="text"
                  placeholder="Enter your full name"
                  className="text-sm outline-none px-2 bg-transparent text-white border-0"
                  {...register("fullName")}
                />
                <button
                  type="submit"
